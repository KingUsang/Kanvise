...
export function StudentAssignmentsClient({ assignments: initialAssignments }: { assignments: StudentAssignment[] }) {
  ...
  return <main className="mx-auto max-w-[1440px] px-4 py-7 pb-24 sm:px-6 lg:px-10 lg:py-10">
    <header><p className="text-sm font-medium text-[#994704]">Coursework</p><h1 className="mt-1 text-3xl font-semibold tracking-tight">Assignments</h1><p className="mt-2 text-sm text-[#716c76]">Review instructions, submit your work, and see feedback from your tutors.</p></header>
    <div className="mt-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="flex overflow-x-auto rounded-xl bg-[#eeeae6] p-1">{(["all", "pending", "submitted", "graded", "overdue"] as Status[]).map((item) => <button key={item} onClick={() => setStatus(item)} className={`whitespace-nowrap rounded-lg px-4 py-2 text-sm font-medium capitalize ${status === item ? "bg-white text-[#2e2877] shadow-sm" : "text-[#716c76]"}`}>{item}</button>)}</div><label className="relative block lg:w-80"><Search className="absolute left-3 top-3 text-[#8b858f]" size={17} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search by title or course" className="w-full rounded-xl border border-[#ddd7d2] bg-white py-2.5 pl-10 pr-3 text-sm outline-none focus:border-[#2e2877]" /></label></div>
    <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(300px,0.85fr)_minmax(420px,1.4fr)]">
      <section className="space-y-3">{visible.length ? visible.map((item) => { const current = assignmentStatus(item); return <button key={item.id} onClick={() => setSelectedId(item.id)} className={`w-full rounded-2xl border bg-white p-4 text-left transition ${selectedId === item.id ? "border-[#2e2877] shadow-sm" : "border-[#e5e1dd] hover:border-[#bcb5c1]"}`}><div className="flex items-start justify-between gap-3"><span className="text-[10px] font-bold uppercase tracking-wide text-[#994704]">{item.course?.name || "Course"}</span><span className={`rounded-full px-2 py-1 text-[10px] font-bold uppercase ${current === "graded" ? "bg-[#e7f4ec] text-[#267045]" : current === "submitted" ? "bg-[#eeeafe] text-[#2e2877]" : current === "overdue" ? "bg-[#fde8e4] text-[#a43522]" : "bg-[#f7eadc] text-[#994704]"}`}>{current}</span></div><h2 className="mt-2 font-semibold">{item.title}</h2><p className="mt-3 flex items-center gap-1.5 text-xs text-[#716c76]"><CalendarDays size={13} />Due {dateTime(item.deadline_at)}</p></button>; }) : <div className="rounded-2xl border border-[#e5e1dd] bg-white py-12 text-center"><FileText className="mx-auto text-[#aaa4ad]" /><p className="mt-3 font-medium">No assignments found</p><p className="mt-1 text-sm text-[#716c76]">Try another filter or search.</p></div>}</section>
      {selected ? <section className="fixed inset-0 z-50 overflow-y-auto bg-[#f8f7f5] p-4 lg:sticky lg:top-24 lg:z-0 lg:max-h-[calc(100vh-7rem)] lg:rounded-2xl lg:border lg:border-[#e5e1dd] lg:bg-white lg:p-6"><button onClick={() => setSelectedId(null)} className="float-right rounded-lg p-2 lg:hidden" aria-label="Close assignment"><X /></button><p className="text-xs font-semibold uppercase tracking-wide text-[#994704]">{selected.course?.name || "Course"}</p><h2 className="mt-2 pr-10 text-2xl font-semibold">{selected.title}</h2><p className="mt-2 text-sm text-[#716c76]">Due {dateTime(selected.deadline_at)}</p><div className="mt-6 border-t border-[#eeeae6] pt-5"><h3 className="font-semibold">What you need to do</h3><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-[#5f5964]">{selected.description}</p></div>{selected.attachment_download_url && <a href={selected.attachment_download_url} target="_blank" rel="noreferrer" className="mt-5 flex items-center justify-between rounded-xl bg-[#f3f0ed] p-4 text-sm font-medium text-[#2e2877]"><span className="flex min-w-0 items-center gap-2"><FileText size={18} /><span className="truncate">{selected.attachment_file_name || "Assignment resource"}</span></span><Download size={17} /></a>}
      ...
      </section> : <section className="hidden rounded-2xl border border-[#e5e1dd] bg-white p-8 text-center lg:block"><FileText className="mx-auto text-[#aaa4ad]" /><p className="mt-3 font-medium">Choose an assignment</p><p className="mt-1 text-sm text-[#716c76]">Its instructions and submission area will appear here.</p></section>}
    </div>
  </main>;
}
