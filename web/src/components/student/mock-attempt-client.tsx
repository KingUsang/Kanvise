...
export function MockAttemptClient({ data, token }: { data: AttemptData; token: string }) {
  ...
  const sectionTitles = useMemo(() => [...new Set(questions.map((question) => question.section_title || 'Questions'))], [questions])
  const activeSectionTitle = active?.section_title || 'Questions'
  const activeSectionQuestions = useMemo(() => questions.map((question, index) => ({ question, index })).filter(({ question }) => (question.section_title || 'Questions') === activeSectionTitle), [activeSectionTitle, questions])
  const currentSectionPosition = Math.max(0, activeSectionQuestions.findIndex(({ index }) => index === current))
  const remaining = data.attempt.deadline_at
    ? Math.max(0, Math.ceil((new Date(data.attempt.deadline_at).getTime() - (now + serverOffset)) / 1000)) : null
  ...
  }, [answers, data.attempt.id, save, timedOut])
  ...
  useEffect(() => {
    if (!timedOut) return
    if (isOnline && !timeoutSubmitted.current) {
      timeoutSubmitted.current = true
      void submit().then((succeeded) => {
        if (!succeeded) timeoutSubmitted.current = false
      })
    }
  }, [isOnline, submit, timedOut])
  ...
  }, [data.attempt.id, questions, save, timedOut])
  ...
  useEffect(() => {
    function keyboard(event: KeyboardEvent) {
      ...
      if (!timedOut && optionIndex >= 0 && active?.question_type === 'mcq' && active.options[optionIndex]) {
        changeAnswer(active.id, { selected_option_version_id: active.options[optionIndex].id })
      }
    }
    window.addEventListener('keydown', keyboard)
    return () => window.removeEventListener('keydown', keyboard)
  }, [active, changeAnswer, confirming, questions.length, timedOut])
  ...
  return <main className="min-h-[calc(100vh-4rem)] bg-[#f8f7f5] pb-28 lg:pb-8">
    ...
    {sectionTitles.length > 1 && <nav aria-label="Mock subjects" className="border-b border-[#e2ddd8] bg-white px-4 py-3 sm:px-6 lg:px-10"><div className="mx-auto flex max-w-[1440px] gap-2 overflow-x-auto pb-1">{sectionTitles.map((title) => { const section = questions.map((question, index) => ({ question, index })).filter(({ question }) => (question.section_title || 'Questions') === title); const sectionAnswered = section.filter(({ question }) => { const saved = answers.get(question.id); return Boolean(saved?.selected_option_version_id || saved?.theory_answer_text?.trim()) }).length; return <button key={title} type="button" onClick={() => setCurrent(section[0]?.index || 0)} aria-current={title === activeSectionTitle ? 'page' : undefined} className={`min-w-max rounded-lg border px-3 py-2 text-left text-sm ${title === activeSectionTitle ? 'border-[#2e2877] bg-[#eeeafe] font-semibold text-[#2e2877]' : 'border-[#ddd7d2] bg-white text-[#5f5964]'}`}><span className="block">{title}</span><span className="mt-0.5 block text-[11px] opacity-75">{sectionAnswered}/{section.length} answered</span></button> })}</div></nav>}
    <div className="mx-auto grid max-w-[1440px] gap-5 px-4 py-5 sm:px-6 lg:grid-cols-[230px_minmax(0,1fr)] lg:px-10">
      <aside className="order-2 lg:order-1"><div className="sticky top-36 rounded-2xl border border-[#e2ddd8] bg-white p-4"><div className="flex items-center justify-between"><div><h2 className="text-sm font-semibold">{sectionTitles.length > 1 ? activeSectionTitle : 'Questions'}</h2><p className="mt-0.5 text-[11px] text-[#716c76]">{answeredCount}/{questions.length} total answered</p></div><span className="text-xs text-[#716c76]">{currentSectionPosition + 1}/{activeSectionQuestions.length}</span></div><div className="mt-4 grid grid-cols-[repeat(auto-fill,minmax(2.5rem,1fr))] gap-2 lg:grid-cols-5">{activeSectionQuestions.map(({ question, index }, sectionIndex) => { const saved = answers.get(question.id); const answered = Boolean(saved?.selected_option_version_id || saved?.theory_answer_text?.trim()); return <button key={question.id} onClick={() => setCurrent(index)} aria-label={`${activeSectionTitle} question ${sectionIndex + 1}${saved?.is_flagged ? ', flagged' : ''}${answered ? ', answered' : ''}`} className={`relative h-10 min-w-10 rounded-lg border text-xs font-semibold ${index === current ? 'border-[#2e2877] bg-[#2e2877] text-white' : answered ? 'border-[#9bceb0] bg-[#edf8f1] text-[#276744]' : 'border-[#d9d3cf] bg-white text-[#5f5964]'}`}>{sectionIndex + 1}{saved?.is_flagged && <span className="absolute -right-1 -top-1 h-2.5 w-2.5 rounded-full bg-[#c26627]" />}</button>})}</div>{!timedOut && <button onClick={() => setConfirming(true)} className="mt-5 hidden min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#994704] text-sm font-semibold text-white lg:flex"><Send size={16} />Review and submit</button>}</div></aside>
      <section className="order-1 min-w-0 lg:order-2"><article className="rounded-2xl border border-[#e2ddd8] bg-white p-5 sm:p-7 lg:p-9">
        <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-wider text-[#994704]">{sectionTitles.length > 1 ? `${activeSectionTitle} · ` : ''}Question {currentSectionPosition + 1} of {activeSectionQuestions.length}</p><p className="mt-1 text-xs text-[#716c76]">{active.marks} mark{Number(active.marks) === 1 ? '' : 's'}</p></div><button disabled={timedOut} onClick={() => changeAnswer(active.id, { is_flagged: !answer.is_flagged })} className={`inline-flex min-h-10 items-center gap-2 rounded-lg border px-3 text-sm font-medium disabled:cursor-not-allowed disabled:opacity-50 ${answer.is_flagged ? 'border-[#c26627] bg-[#fff4e8] text-[#994704]' : 'border-[#d9d3cf] text-[#716c76]'}`}><Flag size={16} fill={answer.is_flagged ? 'currentColor' : 'none'} />{answer.is_flagged ? 'Flagged' : 'Flag'}</button></div>
        ...
        {active.question_type === 'mcq' ? <fieldset disabled={timedOut} className="mt-7 space-y-3"><legend className="sr-only">Choose one answer</legend>{active.options.map((option, index) => { const selected = answer.selected_option_version_id === option.id; return <label key={option.id} className={`flex min-h-14 items-start gap-3 rounded-xl border p-4 transition ${timedOut ? 'cursor-not-allowed opacity-70' : 'cursor-pointer'} ${selected ? 'border-[#2e2877] bg-[#f0edff]' : 'border-[#ddd7d2]'} ${!timedOut && !selected ? 'hover:border-[#aaa2b0]' : ''}`}><input type="radio" name={active.id} checked={selected} onChange={() => changeAnswer(active.id, { selected_option_version_id: option.id, theory_answer_text: null })} className="mt-1 h-4 w-4 accent-[#2e2877]" /><span className="flex min-w-0 gap-2"><span className="font-semibold text-[#2e2877]">{String.fromCharCode(65 + index)}.</span><QuestionContent plainText={option.plain_text} blocks={option.content_blocks} /></span></label>})}</fieldset>
          : <div className="mt-7"><label className="text-sm font-semibold" htmlFor={`answer-${active.id}`}>Your answer</label><textarea disabled={timedOut} id={`answer-${active.id}`} value={answer.theory_answer_text || ''} onChange={event => changeAnswer(active.id, { theory_answer_text: event.target.value, selected_option_version_id: null }, true)} maxLength={20000} className="mt-2 min-h-56 w-full rounded-xl border border-[#d7d1cc] p-4 text-sm leading-6 outline-none focus:border-[#2e2877] focus:ring-1 focus:ring-[#2e2877] disabled:cursor-not-allowed disabled:bg-[#f3f1ef] disabled:text-[#716c76]" placeholder="Type your answer here…" /><p className="mt-1 text-right text-xs text-[#8b858f]">{(answer.theory_answer_text || '').length.toLocaleString()} / 20,000</p></div>}
        <div className="mt-7 flex items-center justify-between border-t border-[#eeeae6] pt-5"><span className={`flex items-center gap-1.5 text-xs ${timedOut || saveState === 'error' || saveState === 'offline' ? 'text-[#a43522]' : 'text-[#716c76]'}`}>{timedOut ? <><AlertTriangle size={14} />Answers locked at deadline</> : saveState === 'saving' ? 'Saving…' : saveState === 'saved' ? <><Check size={14} />Saved</> : saveState === 'offline' ? <><CloudOff size={14} />Offline — change queued</> : saveState === 'error' ? <><AlertTriangle size={14} />Not saved — retrying</> : 'Changes save automatically'}</span><div className="flex gap-2"><button disabled={current === 0} onClick={() => setCurrent(value => value - 1)} className="inline-flex min-h-11 items-center gap-1 rounded-xl border border-[#d9d3cf] px-4 text-sm font-semibold disabled:opacity-40"><ChevronLeft size={17} />Previous</button><button disabled={timedOut && current === questions.length - 1} onClick={() => current === questions.length - 1 ? setConfirming(true) : setCurrent(value => value + 1)} className="inline-flex min-h-11 items-center gap-1 rounded-xl bg-[#2e2877] px-4 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50">{current === questions.length - 1 ? timedOut ? 'End of mock' : 'Review' : 'Next'}{current < questions.length - 1 && <ChevronRight size={17} />}</button></div></div>
      </article></section>
    </div>
