...
export function StudentMocksClient({ groups }: { groups: StudentMockGroups }) {
  ...
  return <main className="mx-auto max-w-[1440px] px-4 py-7 pb-24 sm:px-6 lg:px-10 lg:py-10">
    ...
    <div className="mt-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
      <div role="group" aria-label="Filter mock exams by status" className="grid grid-cols-2 gap-1 rounded-xl bg-[#eeeae6] p-1 sm:flex">{tabs.map(item => <button type="button" key={item.key} aria-pressed={tab === item.key} onClick={() => setTab(item.key)} className={`min-h-10 min-w-0 rounded-lg px-3 py-2 text-sm font-medium sm:px-4 ${tab === item.key ? 'bg-white text-[#2e2877] shadow-sm' : 'text-[#716c76]'}`}>{item.label}<span className="ml-2 rounded-full bg-black/5 px-1.5 py-0.5 text-[10px]">{groups[item.key].length}</span></button>)}</div>
