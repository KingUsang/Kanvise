...
export function StudentClassesClient({ classes }: { classes: StudentClass[] }) {
  ...
  const courses = useMemo(() => Array.from(new Map(classes.map((item) => [item.course_id, item.course?.name || "Course"])).entries()), [classes]);
  ...
  return <>
    ...
    <section className="mt-6 rounded-2xl border border-[#e5e1dd] bg-white p-4 sm:p-6">
      <div className="flex flex-col gap-4 border-b border-[#eeeae6] pb-5 sm:flex-row sm:items-center sm:justify-between">
        ...
        <label className="flex items-center gap-2 text-sm text-[#716c76]"><span className="shrink-0">Course</span><select value={courseId} onChange={(event) => setCourseId(event.target.value)} className="min-w-0 flex-1 rounded-xl border border-[#dcd6d1] bg-white px-3 py-2.5 text-[#25232d] outline-none focus:border-[#2e2877] sm:min-w-52"><option value="all">All courses</option>{courses.map(([id, name]) => <option key={id} value={id}>{name}</option>)}</select></label>
      </div>
      ...
      <div className="mt-5 space-y-4">
        {visibleClasses.length ? visibleClasses.map((item) => {
          ...
          return <article key={item.id} className={`rounded-2xl border p-4 sm:flex sm:items-center sm:gap-5 sm:p-5 ${item.status === "live" ? "border-[#c26627] bg-[#fffaf5]" : "border-[#e5e1dd] bg-white"}`}>
            ...
            <div className="mt-4 min-w-0 flex-1 sm:mt-0"><div className="flex flex-wrap items-center gap-2"><p className="font-semibold">{item.title}</p>{item.status === "live" && <span className="rounded-full bg-[#fbe6d6] px-2 py-1 text-[10px] font-bold uppercase tracking-wide text-[#994704]">Live now</span>}{item.status === "cancelled" && <span className="rounded-full bg-[#f1efed] px-2 py-1 text-[10px] font-bold uppercase tracking-wide text-[#716c76]">Cancelled</span>}</div><p className="mt-1 text-sm text-[#716c76]">{item.course?.name || "Course"}</p><p className="mt-2 flex items-center gap-1.5 text-xs text-[#77727e]"><UserRound size={13} />{tutorName(item)}</p></div>
