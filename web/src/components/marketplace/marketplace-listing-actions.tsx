...
export function MarketplaceListingActions({ listingId, listingSlug, free }: { listingId: string; listingSlug?: string; free: boolean }) {
  const router = useRouter()
  const searchParams = useSearchParams()
  const [loading, setLoading] = useState(false)
  const autoClaimStarted = useRef(false)
  const redirect = () => `/mocks/${encodeURIComponent(listingSlug || (typeof window !== 'undefined' ? window.location.pathname.split('/').filter(Boolean).pop() || '' : ''))}?claim=${encodeURIComponent(listingId)}`
  ...
  useEffect(() => {
    if (!free || searchParams.get('claim') !== listingId || autoClaimStarted.current) return
    autoClaimStarted.current = true
    void claim()
  }, [free, listingId, searchParams])
