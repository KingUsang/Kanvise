...
export function ProgrammeBuilder({ programmeId }: { programmeId?: string }) {
  ...
  if (loading) return <div className="mx-auto max-w-[1440px] rounded-lg border border-[#c8c5d2] bg-white p-12 text-center text-sm text-[#474551]">Loading programme setup…</div>
    ...
    <div className="mx-auto w-full max-w-[1440px]">
      ...
      <div className="grid grid-cols-12 gap-6">
        <aside className="col-span-3 hidden lg:block">
          <ol className="sticky top-6 space-y-2 rounded-lg border border-[#c8c5d2] bg-white p-4 shadow-sm">
        ...
        <main className="col-span-12 lg:col-span-9">
          <section className="rounded-lg border border-[#c8c5d2] bg-white shadow-sm">
...
function DetailsStep({ draft, coverFile, payoutReady, updateDraft, setCoverFile }: { draft: ProgrammeDraftData; coverFile: File | null; payoutReady: boolean | null; updateDraft: <K extends keyof ProgrammeDraftData>(key: K, value: ProgrammeDraftData[K]) => void; setCoverFile: (file: File | null) => void }) {
  return <div className="grid grid-cols-12 gap-5">
    ...
    <details className="col-span-12 rounded-lg border border-[#e4e2e1] bg-[#fbf9f8]"><summary className="cursor-pointer list-none px-4 py-3 text-sm font-semibold text-[#2e2877] marker:content-none">Add description or cover image <span className="font-normal text-[#716c76]">(optional)</span></summary><div className="grid gap-5 border-t border-[#e4e2e1] p-4"><div><FieldLabel>Description</FieldLabel><textarea value={draft.description} onChange={event => updateDraft('description', event.target.value)} rows={3} placeholder="Tell students what they will learn and who this programme is for." className="w-full rounded border border-[#c8c5d2] px-3.5 py-3 text-sm outline-none focus:border-[#2e2877]" /></div><div><FieldLabel>Cover image</FieldLabel><label className="flex cursor-pointer items-center gap-3 rounded border border-dashed border-[#c8c5d2] bg-white p-3 text-sm text-[#474551]"><span className="material-symbols-outlined text-[#2e2877]">add_photo_alternate</span><span className="min-w-0 truncate">{coverFile?.name || draft.coverFileName || 'Choose a JPG, PNG or WebP image'}</span><input type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" onChange={event => setCoverFile(event.target.files?.[0] || null)} /></label>{draft.coverFileName && !coverFile && <p className="mt-2 text-xs text-amber-700">Select this image again before saving; files are kept only for this browser session.</p>}</div></div></details>
  </div>
}
