  ...
  return (
    <nav aria-label="Dashboard navigation" className="fixed inset-x-0 bottom-0 z-40 border-t border-[#e4e2e1] bg-white px-2 pb-[max(0.45rem,env(safe-area-inset-bottom))] pt-1.5 shadow-[0_-8px_24px_rgba(35,31,38,0.10)] md:hidden">
      ...
      {items.map((item) => {
        ...
        return (
          <Link key={item.href} href={item.href} aria-current={isActive ? 'page' : undefined} className={`flex min-h-12 flex-col items-center justify-center gap-0.5 px-1 text-[10px] font-semibold leading-none transition-colors ${isActive ? 'text-[#2e2877]' : 'text-[#77747e] hover:text-[#2e2877]'}`}>
            <span className={`material-symbols-outlined flex h-8 w-10 shrink-0 items-center justify-center self-center rounded-full text-[21px] leading-none ${isActive ? 'icon-fill bg-[#eeecfa] text-[#2e2877]' : ''}`}>{item.icon}</span>
