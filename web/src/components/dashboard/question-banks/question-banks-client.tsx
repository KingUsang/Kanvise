...
export function buildQuestionContent(plainText: string, blockType: 'none' | 'equation' | 'chemistry', latex: string): ContentBlock[] {
  const blocks: ContentBlock[] = [{ type: 'text', text: plainText.trim() }]
                  ...
                  <article key={question.id} className="p-5 transition hover:bg-[#fcfaf8] md:p-6">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {question.current_version.content_blocks.filter((block): block is Extract<ContentBlock, { type: 'equation' | 'chemistry' }> => block.type !== 'text').map((block, blockIndex) => <ScientificBlock key={blockIndex} block={block} />)}
