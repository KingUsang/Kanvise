...
export function DashboardClassLauncher({
  ...
  const startClass = async () => {
    ...
    try {
      ...
      if (accessMode === 'anyone_with_link' && body.data.share_token) {
        setShareUrl(`${window.location.origin}/live/${body.data.share_token}`)
        setShareClassId(body.data.id)
        setStarting(false)
        toast.success('Your class is ready to share')
        return
      }
  ...
  return <>
    ...
    {open ? <div className="fixed inset-0 z-[70] flex items-end bg-black/45 p-0 sm:items-center sm:justify-center sm:p-5" role="dialog" aria-modal="true" aria-labelledby="start-class-title">
      ...
      <section className="relative w-full rounded-t-3xl bg-white p-5 shadow-2xl sm:max-w-md sm:rounded-2xl sm:p-7">
        ...
        {shareUrl ? <div className="mt-6"><p className="text-sm leading-5 text-[#66616c]">Share this link. Anyone with it can join while your class is live.</p><input readOnly value={shareUrl} className="mt-4 min-h-12 w-full rounded-lg border border-[#8b8580] bg-[#fbf9f8] px-3 text-sm" /><div className="mt-4 grid grid-cols-2 gap-3"><button type="button" onClick={() => void navigator.clipboard.writeText(shareUrl).then(() => toast.success('Class link copied'))} className="min-h-11 rounded-lg border border-[#2e2877] px-3 text-sm font-semibold text-[#2e2877]">Copy link</button><button type="button" disabled={!shareClassId} onClick={() => { if (!shareClassId) return; startNavigationProgress(); router.push(`/class/${shareClassId}?start=true`) }} className="min-h-11 rounded-lg bg-[#994704] px-3 text-sm font-semibold text-white disabled:opacity-50">Enter class</button></div></div> : <>
