...
type PublicClass = { title: string; status: string; scheduled_at: string; centre_name: string; centre_logo_url: string | null }
...
export default function PublicClassroom({ shareToken }: { shareToken: string }) {
  ...
  const join = async () => {
    ...
    try {
      ...
      const body = await response.json().catch(() => null)
      if (!response.ok) { setError(body?.error || 'Could not join this class'); if (response.status === 409) void load(); return }
      setJoined(body.data)
