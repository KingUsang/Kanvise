...
export default async function StudentDashboardPage() {
  ...
  if (data.capabilities?.hasCentreLearning === false) {
    ...
    const completed = data.standalone_mocks?.attempts_completed || 0;
    const latestResult = data.standalone_mocks?.latest_result || null;
    return <main className="mx-auto max-w-[1120px] px-4 py-7 pb-24 sm:px-6 lg:px-10 lg:py-10">
      <section className="rounded-3xl bg-[#2e2877] px-6 py-8 text-white sm:px-9 sm:py-10"><p className="text-sm text-white/70">Kanvise</p><h1 className="mt-2 text-3xl font-semibold tracking-tight sm:text-4xl">Welcome, {firstName}</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-white/75">Open a mock from the link you received, practise at your own pace, and keep every result in one place.</p><Link href="/dashboard/student/mocks?view=unlocked" className="mt-6 inline-flex min-h-11 items-center rounded-xl bg-white px-4 py-2.5 text-sm font-semibold text-[#2e2877]">Open mocks <ArrowRight className="ml-2" size={16}/></Link></section>
      <section className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3"><article className="rounded-2xl border border-[#e3ded9] bg-white p-5"><p className="text-xs font-semibold uppercase tracking-wide text-[#716c76]">Unlocked mocks</p><p className="mt-2 text-3xl font-semibold text-[#2e2877]">{owned}</p><p className="mt-2 text-sm text-[#716c76]">Free and purchased practice mocks.</p><Link href="/dashboard/student/mocks?view=unlocked" className="mt-4 inline-flex text-sm font-semibold text-[#2e2877]">Open mocks</Link></article><article className="rounded-2xl border border-[#e3ded9] bg-white p-5"><p className="text-xs font-semibold uppercase tracking-wide text-[#716c76]">Continue practising</p><p className="mt-2 text-3xl font-semibold text-[#2e2877]">{inProgress}</p><p className="mt-2 text-sm text-[#716c76]">Attempt{inProgress === 1 ? '' : 's'} currently in progress.</p><Link href="/dashboard/student/mocks?view=unlocked" className="mt-4 inline-flex text-sm font-semibold text-[#2e2877]">{inProgress ? 'Continue a mock' : 'Choose a mock'}</Link></article><article className="rounded-2xl border border-[#e3ded9] bg-white p-5 sm:col-span-2 xl:col-span-1"><p className="text-xs font-semibold uppercase tracking-wide text-[#716c76]">Completed attempts</p><p className="mt-2 text-3xl font-semibold text-[#2e2877]">{completed}</p>{latestResult ? <><p className="mt-2 truncate text-sm text-[#716c76]">Latest: {latestResult.title} · {latestResult.percentage === null ? 'Marking in progress' : `${latestResult.percentage}%`}</p><Link href={`/result/${latestResult.id}`} className="mt-4 inline-flex text-sm font-semibold text-[#2e2877]">View latest result</Link></> : <p className="mt-2 text-sm text-[#716c76]">Your completed mock results will appear here.</p>}</article></section>
      ...
      <div className="space-y-6">
        <section className="rounded-2xl border border-[#e5e1dd] bg-white p-5 sm:p-6">
          ...
          {nextClass ? <div className="mt-5 rounded-2xl border-l-4 border-[#c26627] bg-[#faf8f5] p-4 sm:flex sm:items-center sm:justify-between sm:gap-5">
            <div className="flex min-w-0 gap-4"><div className="min-w-16 shrink-0 border-r border-[#ded8d2] pr-4"><p className="text-lg font-semibold text-[#2e2877]">{formatTime(nextClass.scheduled_at)}</p><p className="text-xs text-[#77727e]">{formatDate(nextClass.scheduled_at)}</p></div><div className="min-w-0"><p className="break-words font-semibold">{nextClass.title}</p><p className="mt-1 break-words text-sm text-[#716c76]">{nextClass.course_name} · {nextClass.duration_minutes} mins</p></div></div>
