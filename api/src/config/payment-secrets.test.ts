...
describe('validateProductionPaymentSecrets', () => {
  it('rejects missing and placeholder production secrets', () => {
    ...
    expect(() => validateProductionPaymentSecrets({
      NODE_ENV: 'production', PAYSTACK_SECRET_KEY: 'paystack-production-secret-abcdefghijklmnopqrstuvwxyz', KANVISE_INTERNAL_SECRET: 'change-me',
  ...
  it('accepts strong production secrets', () => {
    expect(() => validateProductionPaymentSecrets({
      NODE_ENV: 'production',
      PAYSTACK_SECRET_KEY: 'paystack-production-secret-abcdefghijklmnopqrstuvwxyz',
