...
| UX-07 centre slug | Implemented; staging migration pending | Generation/edit warning and reserved routes are protected (`916ea0e`, `a86cf4c`); historical redirects are implemented atomically (`2761cb0`). Supabase dry-run is blocked by remote migration versions absent from this checkout, so the new migration has not been pushed blindly. |
| UX-08–10 student navigation, density and filters | Concurrent branch boundary | Do not duplicate or overwrite `.student-journey-worktree`; final verification must use the integrated student branch. |
