...
Files are served through a Cloudflare R2 public bucket URL or a custom domain (e.g. `cdn.kanvise.com`). Sensitive files (student submissions, private notes) are served via short-lived presigned download URLs generated by the Hono backend. Public files (centre logos, banner images, promo images) are served from a public R2 bucket directly.
...
A custom sending domain must be configured in Resend for production — emails must come from a domain like `noreply@kanvise.com`, not from a Resend subdomain.
