...
Every developer must read this document in full. Every pull request that touches database queries, API routes, or storage operations will be reviewed against the rules defined here. Any code that violates these rules must be rejected regardless of how small or harmless the violation appears.
...
There is one Supabase PostgreSQL database. All tutorial centres share this single database and the same set of tables. Tenant isolation is enforced by a `school_id` column present on every tenant-scoped table. Every query that reads or writes tenant-scoped data must include a `school_id` filter.
...
Separate schemas per tenant would require dynamic schema creation per centre, complex migration management across hundreds of schemas, and connection pool management that Supabase's standard configuration does not support cleanly.

The shared schema model is the standard approach for SaaS at Kanvise's scale. The risk — cross-tenant data leakage — is mitigated entirely by the middleware and query patterns defined in this document. The risk of getting those wrong is lower than the operational risk of managing per-tenant infrastructure at MVP.

---

## 2. What a Tenant Is

In Kanvise, a **tenant is a tutorial centre**. Every tutorial centre that signs up and creates a school on Kanvise is a tenant. The tenant's identifier is the `school_id` — a UUID generated when the Admin creates their school.

Every user (Admin, Tutor, Student) belongs to exactly one school. Their `school_id` is stored in their user profile record and embedded in their JWT as a custom claim at login time.
...
The `ctx.user.school_id` attached by middleware is the authoritative school identifier for the entire request. Route handlers must never accept a `school_id` from the request body or query parameters as the source of tenant identity. The school_id always comes from `ctx.user.school_id` — never from what the client sends.
...
Reads the Authorization header, extracts the Bearer token, verifies the JWT signature using the Supabase JWT secret. If verification fails for any reason — invalid signature, expired token, malformed token — the middleware returns a 401 immediately. The request does not proceed.

### profileResolutionMiddleware

Uses the verified JWT's `sub` claim (Supabase Auth user ID) to query the `user_profiles` table for the user's Kanvise profile. If no profile is found — which should never happen for a valid user but could indicate a partially created account — the middleware returns a 403. The resolved profile is attached to `ctx.user`.

### tenantMiddleware

Confirms that `ctx.user.school_id` is present and is a valid UUID. If it is missing or invalid, the middleware returns a 403. This middleware exists as a safety net — profileResolutionMiddleware should always populate school_id, but this check ensures no request ever reaches a route handler without a confirmed tenant.

### roleMiddleware

Each protected route declares the roles that may access it. The role middleware compares `ctx.user.role` against the route's declared roles. If the user's role is not in the allowed set, the middleware returns a 403. Route-level role declarations are defined alongside the route handler, not in a separate configuration file, so the permission is always visible next to the code it protects.

---

## 5. Database Query Rules

These rules apply to every database query in the Hono codebase. They are mandatory and non-negotiable.
...
When a client requests a specific resource by ID (e.g. GET /programmes/:id), the query must include both the resource ID and the school_id. This prevents a user from accessing a resource that belongs to another school by guessing its ID.
...
// WRONG — courses filtered but programme join could pull from another school
const { data } = await supabase
  .from('courses')
...
The following tables contain school data and every query must include a `school_id` filter.

| Table | school_id Column | Notes |
|---|---|---|
| schools | id (schools IS the tenant table) | Primary tenant anchor |
| user_profiles | school_id | Every user belongs to one school |
| programmes | school_id | |
| sub_programmes | school_id | Derived from programme but independently scoped |
| courses | school_id | |
| enrolments | school_id | Student → programme/course access records |
...
| mock_answers | school_id | Student answers — never returned before submission |
...
The critical rule for public routes: the school_id used for all queries within a public route handler must always come from the database lookup by slug — never from query parameters or request body. The slug is a read-only public identifier. It cannot be used to modify data.
...
Tenant isolation in Cloudflare R2 is enforced through the folder structure. Every file belonging to a school is stored under a path that begins with `schools/{school_id}/`. This means:

A student from School A whose `school_id` is `abc123` cannot construct a URL to a file from School B whose `school_id` is `xyz789` because the path `schools/xyz789/notes/somefile.pdf` is not derivable from anything School A's users can see.

For private files (submissions, assignment attachments), presigned URLs are the only access mechanism. The Hono route that generates a presigned GET URL verifies that the requesting user's `school_id` matches the `school_id` in the file's storage key before generating the URL. If they do not match, the request is rejected with a 403.
...
Tenancy defines which school a user belongs to. Within a school, roles define what a user can do. These two concepts layer on top of each other.
...
Role enforcement happens in route handlers after the tenant middleware has confirmed the school_id. The tenant middleware ensures the user is in the right school. The role check ensures they have the right permissions within that school.
...
app.post('/courses/:courseId/notes', authMiddleware, requireRole('tutor'), async (ctx) => {
...
app.get('/courses/:courseId/notes', authMiddleware, requireRole('student'), async (ctx) => {
  const courseId = ctx.req.param('courseId')

  // Verify student is enrolled in this course (directly or via a programme)
...
A student enrolled in a Programme automatically has access to all Courses and Sub-programmes within that Programme. The enrolment check must account for this — a student enrolled at the Programme level should pass the access check for any course inside that programme without requiring a separate course-level enrolment record.

---

## 12. Violations and What to Do

The following are all violations of the multi-tenancy architecture. If any of these are found in the codebase, they must be treated as critical bugs and fixed before the code is merged.

**Violation 1 — Query without school_id filter on a tenant-scoped table.**
...
**Violation 3 — Resource fetched by ID without tenant verification.**
The resource may belong to another school. Fix by adding `.eq('school_id', ctx.user.school_id)` to the query alongside the ID filter.

**Violation 4 — Write operation in a public route.**
Public routes are read-only. Move the write to an authenticated route.

**Violation 5 — Presigned URL generated without verifying the file's school_id matches the requester's school_id.**
A user from any school could download another school's private files. Fix by extracting and comparing school_id from the file key before generating the URL.

**Violation 6 — Joining tables without scoping the join to the tenant.**
A join could pull in related records from another school. Fix by adding tenant filters to all sides of the join.

**Violation 7 — Student can access content without an enrolment check.**
A student who has not paid could access a school's content. Fix by adding the enrolment verification before returning any course content.
...
No query returns data without a school_id filter unless the table is explicitly listed in Section 7 (platform-level tables).
